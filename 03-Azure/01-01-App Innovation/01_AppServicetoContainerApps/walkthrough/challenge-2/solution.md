...
### **Task 2: Setup a new GitHub Actions pipeline in the repository to build the application**

Go to your GitHub repository and open the *Actions* tab. As you can see, there is already a pipeline called "Build and Deploy" in place that is currently used for the Azure App Service. Click on *New workflow*:

![image](./img/challenge-2-newworkflow.jpg)

Search for *.NET* in the search bar to create the pipeline from a template. Chose the *.NET* template (not to be confused with the *.NET Desktop*) and click *configure*:

![image](./img/challenge-2-createworkflow.jpg)

Rename the pipeline YAML file to *pipeline-containerapp.yml*. As you can see, the template is already filled with some settings and tasks. Let's go through them line by line and make some first changes:

* In line 4 the name of the pipeline is set. Let's rename it to *Build and Deploy to App Service*
* In line 6 to 10 the triggers when the pipeline should run are defined. Currently, every time someone pushes something into the repository or someone makes a pull request the pipeline will run. Since you will for the sake of simplicity only work with the main branch in GitHub directly, you should avoid automatically running the pipeline. Replace the triggers simply with:
        on:
          workflow_dispatch:

* From line 9 onwards the jobs are defined and steps are defined.
* Line 12 tells GitHub to run the pipeline on a Ubuntu Linux machine with the latest available version
* Line 15 checkous the repository
* Line 16 to 20 set up the required .NET tools for the pipeline. Change the *dotnet-version* in line 19 to *7.x*
...
Feel free to name all steps and format the code as you like. Your pipeline should in the end look something like this:
...
Now hit *Commit changes* to save the pipeline. In the dialoge, leave the fields as is and hit *Commit changes*.

![image](./img/challenge-2-commitworkflow.jpg)

Go to the *Actions* tab. As you can see, the new pipeline appears in the list. Chose the *Build and Deploy to Container App* pipeline and click *Run workflow* to test it.

![image](./img/challenge-2-runworkflow.jpg)

The pipeline will run for about a minute. You can have a look in the logs to see what is happening when clicking on the pipeline run. After it has completed, you should see a green mark next to the pipeline run:

![image](./img/challenge-2-workflowtest.jpg)

### **Task 3: Create a Dockerfile and add it into the repository**

In order the build a container image in a pipeline, you need to define the steps how the container image is to be built. This is usually done via a Dockerfile that is then added to the repository. Open the *Code* tab to see your repository and then click on *Add file* -> *Create new file*:
...
### **Task 4: Add steps to the GitHub Actions pipeline to containerize the application and push the image into the container registry**

There are multiple ways to achieve that, this is only one solution. You can use the [azure/acr-build](https://github.com/marketplace/actions/azure-container-registry-build) step to build and push an image to the Container Registry. First of all, you need to create a service principal that will be used by the pipeline step to authenticate and access the Container Registry. Therefore, open the Azure CLI in the portal: https://portal.azure.com/#cloudshell/
...
Next you need to store some information as secrets in GitHub. You could in theory store secret information like password directly in the pipeline YAML file but for security reasons you should avoid this in every case. Instead, create secrets by going to the *Settings* tab, on the left side to *Secrets and variables* -> *Actions* and hit *New repository secret*:
...
Now you can add the task to build and push the container image to the *pipeline-containerapp.yml* file in your repository:
...
Save and commit the changes and run the pipeline to check if everything works. After the pipeline completed, open the container registry, under the tab *Repositories* you should see the "microhackapp" repository, open it to see your container images:
