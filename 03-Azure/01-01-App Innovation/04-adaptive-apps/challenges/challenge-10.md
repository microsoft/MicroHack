...
Challenges 04-09 built the Adaptive Apps story from the platform side. You authored
...
> graph as a claim you have to verify against the source and against Challenges 04-09.
...
- Map the trading application's components onto predefined `Radius.*` resource types and
  compare them with the `Radius.Resources/*` contracts from Challenge 04.
...
- Contrast an ephemeral, workflow-hosted Radius control plane with the persistent control
  planes from Challenges 03-09.
- Judge a preview capability honestly, including the parts of the portability problem it
  does not address.

## Fixed target map

Challenge 10 adds a third deployment path. It does not replace the first two.
...
| Challenge 10, Canvas | Ephemeral `radius-cp` k3d cluster created inside GitHub Actions | `aks-adaptive-apps` | `trading-canvas` (new) | `.radius/app.bicep` |
...
Radius control plane you registered recipes with in Challenge 05, and it does not see your
...
- Deploy the Canvas application into the dedicated `trading-canvas` namespace. Do not
  reuse or overwrite the Challenge 06 application namespace.
- Do not modify `iac/app.bicep`, the Challenge 04 resource types, or the Challenge 05
...
- Do not delete the AKS cluster, the lab resource group, or the Challenge 06 deployment.
...
Challenge 06. The Challenge 06 model is `iac/app.bicep`; if you completed Challenge 09,
...
For each row, record the Canvas resource type, the Challenge 06 resource type, and whether
they express the same intent.

Then answer:

- Which Canvas types are predefined `Radius.*` types, and which of the Challenge 04
...
- Where the modeler had to invent a type because no predefined type matched, compare the
  generated type with the Challenge 04 contract for the same capability. Do they have the
  same name, the same inputs, and the same outputs? Would a recipe written for one satisfy
  the other?
- Which Challenge 06-09 capabilities does the generated model not contain? For each one,
...
- How does that compare with `iac/recipes/postgres-azure-flex.bicep`, which you authored,
  published, and registered yourself in Challenge 05? Compare the contract inputs and
  outputs, who initializes the schema, how TLS and network access are handled, and who owns
  the credential.
- Challenge 05 taught that an environment decides how a requirement is met. Point to the
...
the Challenge 06 application is still healthy in its own namespace and still serves
traffic, and that the Challenge 06 Radius control plane does not list the Canvas
...
- What Canvas measurably made easier for the Azure managed-services path, compared with
  the work you did in Challenges 04, 05, and 06.
...
- A completed mapping table compares the generated model with the Challenge 06 model, and
  ...
  a type, the team can say whether it is interchangeable with the Challenge 04 contract for
  the same capability.
...
- The Challenge 06 deployment is still serving traffic, its Radius control plane does not
...
2. The Planned view resolves an implementation for a target you have already chosen. That
   is not the same property as the one Challenge 06 proved.
