...
The teaching value of this challenge is the contrast, not the tool. Challenges 04-09 built
...
1. **Canvas is not a second way to reach the Challenge 06 environment.** It creates its own
...
2. **Canvas does not make the application portable.** Resource types, recipes, and
   environments do that, and they are Challenge 04 and Challenge 05 work. Canvas changes
...
| --- | --- | --- | --- | --- |
| Challenges 06-09, Azure | Persistent `ws-azure-prod` / `env-azure-prod` | `aks-adaptive-apps` | Challenge 06 application namespace | `iac/app.bicep` |
| Challenges 06-09, Local | Persistent `ws-local-prod` / `env-local-prod` | `k3s-azure-vm` | Challenge 06 application namespace | `iac/app.bicep` |
| Challenge 10, Canvas | Ephemeral `radius-cp` k3d cluster in the workflow run | `aks-adaptive-apps` | `trading-canvas` | `.radius/app.bicep` |
...
- **No route.** The modeler adds ingress only on explicit evidence. A Dockerfile `EXPOSE`,
  ...
  deliberately not sufficient. Challenge 06 also declared no ingress and used
...
fixed allow-list of predefined `Radius.*` types. The Challenge 04 catalog exists precisely
because that allow-list does not cover everything a real platform needs.

### Model mapping

| Component | Canvas generated type | Challenge 06-09 type | Same intent? |
| --- | --- | --- | --- |
...
| Container builds | `Radius.Compute/containerImages` built from each Dockerfile | None; Challenge 06 consumes published `ghcr.io/microsoft/adaptive-apps` images | **No.** Canvas builds from source at a pinned commit; Challenge 06 deploys a published tag, which defaults to the mutable `latest` |
...
| AI model | `Radius.AI/models` if the source shows a model endpoint | `Radius.Resources/aiModels`, declared in `iac/ai.bicep` (Challenge 09), not in `iac/app.bicep` | Similar intent, different contract |
...
> [!NOTE]
> The AI row compares against `iac/ai.bicep`. Challenge 06's `iac/app.bicep` deliberately
> keeps AI disabled. A team that skipped Challenge 09 should record the row as out of scope
...
application actually uses; the wider Challenge 04 catalog has further gaps, and listing
them all is an optional extension rather than the point.

**Is the generated custom type interchangeable with the Challenge 04 contract?**
...
the same namespace the team used in Challenge 04 — arriving there from the other direction.
A shared namespace is not a shared contract. The generated type will have its own name,
inputs, and outputs, derived from what the source appeared to need, and the Challenge 05
recipes were written against the team's schema. A recipe written for one does not implement
the other, and the ephemeral control plane cannot see the Challenge 05 registrations in any
...
| OIDC sign-in configuration (Challenge 07) | Configuration, not structure; it is parameters on a container, not a modeled resource |
| Service-mesh authorization (Challenge 08) | A platform policy concern that does not appear in application source at all |
| MQTT authentication method | A property of the broker recipe, which is why Challenge 06 read it from the resource rather than hard-coding it |
...
Do not select the Challenge 06 namespace.
...
- The database resolves to an Azure managed PostgreSQL offering through the `azure-avm`
  pack, built on Azure Verified Modules. Teams should recognise the shape from Challenge 05,
...
- Compared with `iac/recipes/postgres-azure-flex.bicep`, do not let teams stop at
  ...
  owns schema initialization and the `Demo Account` seed that Challenge 06 verified, names
  ...
  Azure service. Ask what would break in Challenge 06's validation if you swapped one recipe
  for the other.
- The decision point is the environment. In Challenge 05 it was `rad recipe register`
...
## Stage 4: Deploy and compare with Challenge 06
...
> Challenges 05 and 06 arriving from a different direction. Coach teams to capture the
...
Step 10 is the punchline: it is the same `rad deploy` from Challenge 06, in a different
...
| Question | Challenge 06 | Challenge 10 |
| --- | --- | --- |
...
kubectl get all -n <challenge-06-namespace>
```
...
The Challenge 06 control plane must still list `adaptive-apps` and must not list the Canvas
application. Also confirm the Challenge 06 application still serves traffic rather than
...
1. **Reported but not judged.** Challenge 07 turned OIDC sign-in on entirely through
   ...
   shape of problem, and Challenge 06 relied on that probe to prove database access.
...
- No hand-authoring of a contract, a recipe, or an OCI publish step to reach an Azure
  managed database. Challenge 04 plus Challenge 05 collapsed into an upstream recipe pack.
...
| --- | --- |
| Preview supports containerized applications deployed to Azure; AWS is stated as coming soon | `env-local-prod` on K3s is not reachable. The whole Local half of Challenges 02-09 is out of scope |
| Requires a Dockerfile | Fine here; a blocker for the brownfield work in Challenge 11 |
...
choose the implementation. That is Challenge 04 and Challenge 05, and it is unchanged by
...
- A recipe pack for that platform. The team already has the substance of one: the
  Challenge 05 K3s recipe set.
- A path to a control plane that does not assume a GitHub-hosted runner can reach the
  cluster. The Challenge 02 topology puts the K3s API behind a localhost Bastion tunnel on
...
keep the Challenge 06 path as the mechanism of record for any application that must also
...
3. Confirm the Challenge 06 application and the AKS cluster are intact.
...
- The completed model mapping table, including the contracts with no predefined equivalent
  ...
  Challenge 04 contract for the same capability.
...
- The workflow run, the ten reconstructed steps, and the Challenge 06 comparison table.
...
- Namespace isolation output from both namespaces, a Challenge 06 application health check
