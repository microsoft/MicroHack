# Walkthrough Challenge 2 - Monitoring and Performance on Azure SQL Managed Instance
...
[4. Optional: Using Github Copilot to solve performance issues](#optional-using-github-copilot-to-solve-performance-issues)
...
We have seen that the legacy application is a multi-tenant system with a collection of 4 databases supporting the transactional workload for each customer – these are the 4 databases we have modernised by migrating to the shared Azure SQL Managed Instance.

Apart from the tenant centric transactional databases there are 2 shared databases:

* **SharedMasterDataDB**: A centralised Data Warehouse database that combines data from the various tenant transactional databases and is used for aggregated reporting and analytics.
...
|**Open** the SQL script:  C:\\MicroHack\\LABS\\02-Administering_Monitoring\\Part_01_Monitoring_Lab_1.sql  <br>    <br>  **Note:** Make sure you connect with the **TenantCRM** database.    <br>  <br>  Run the **PART 1** QUERY 1.  Look at the results and note the [avg_cpu_percent] value is spiking many times indicating that this database is consuming a lot of the hosts CPU resources|![](../../Images/f813232846539c9b2785ea44f8883e94.png)| [[sys].[dm_db_resource_stats]](https://docs.microsoft.com/en-us/sql/relational-databases/system-dynamic-management-views/sys-dm-db-resource-stats-azure-sql-database?view=azuresqldb-current) exposes a number of key database performance metrics.  One metric every 15 seconds over the last hour.|
|Now let’s try the second query to analyze the resource consumption.  <br>  **Execute** QUERY 2 and analyze the values during the last minutes. You will see spikes in CPU consumption.|| If more than 1 hour of statistics is needed, use the DMV [[sys].[server_resource_stats]](https://docs.microsoft.com/en-us/sql/relational-databases/system-catalog-views/sys-server-resource-stats-azure-sql-database?view=azuresqldb-current) up to 14 days|
|**Open** the SQL script:  C:\\MicroHack\\LABS\\02-Administering_Monitoring\\Part_02_Monitoring_Lab_1.sql  <br>    <br>  **Note:** Make sure you connect with the **TenantCRM** database.    <br>  <br>  **Run** the **PART 2** QUERY 1.<br> Identify the most consuming stored procedure based on the total_worker_time (CPU)|![](../../Images/ad69a5f2a781f8c634d6522000027a1a.png)||
...
## Optional: Using Github Copilot to solve performance issues

In this exercise you will leverage Github Copilot to identify performance issues and try mitigate those.

|**Narrative/Instructions**| **Screenshot**| **Notes**|
|:------------|:--------------|:---------|
|On your Team Win11 VM open **SQL Server Management Studio (SSMS)** and connect to the Azure SQL Managed Instance using these details from the readme.txt file. **Authentication: SQL Login** **Login:** **_Will be provided during hack_**<br> **Password:**  **_Will be provided during hack_**| **![](../../Images/4b05bbceae6c4c0d2c4e37e53e60dfd4.png)**||
|Enter a prompt similar to the following to the Github Copilot Chat window *Please check for queries or stored procedures with the highest load in the TenantCRM database and suggest solutions for possible performance issues.*|![](../../Images/GithubCopilot_performance_prompt.png)||
...
|Github Copilot identified the missing index and suggests to create the index. You can insert the **INDEX CREATE** script and execute it in the query window.|![](../../Images/GithubCopilot_MissingIndex.png)||

We have seen how Github Copilot can help us analyze and mitigate performance issues.
