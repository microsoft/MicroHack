...
<tr><td>Select <b>New connection</b> and paste the Data Lake Storage URL in this format: <code>https://employeedataDDMM.dfs.core.windows.net/</code> (1). Here, <code>DDMM</code> is the two-digit deployment day followed by the two-digit deployment month. Use the deployment date for your environment, not today's date. For example, an environment deployed on September 7 uses <code>https://employeedata0709.dfs.core.windows.net/</code>. Click <b>Next (2)</b> to continue.</td><td>Confirm the deployment date with the lab facilitator or deployment output if needed.</td></tr>
