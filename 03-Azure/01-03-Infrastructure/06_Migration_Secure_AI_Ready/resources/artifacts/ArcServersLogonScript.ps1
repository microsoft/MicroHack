...
if ($Env:flavor -ne 'DevOps') {
    ...
    #Invoke-WebRequest ($Env:templateBaseUrl + 'artifacts/prepareSqlServerForAssessment.ps1') -OutFile $nestedVMMHBoxDir\prepareSqlServerForAssessment.ps1
    #Copy-VMFile $SQLvmName -SourcePath "$Env:MHBoxDir\prepareSqlServerForAssessment.ps1" -DestinationPath "$nestedVMMHBoxDir\prepareSqlServerForAssessment.ps1" -CreateFullPath -FileSource Host -Force
    #Invoke-Command -VMName $SQLvmName -ScriptBlock { powershell -File $Using:nestedVMMHBoxDir\prepareSqlServerForAssessment.ps1 } -Credential $winCreds

    # Copy installation script to nested Windows VMs
    #Write-Output 'Transferring installation script to nested Windows VMs...'
    #Copy-VMFile $SQLvmName -SourcePath "$agentScript\installArcAgent.ps1" -DestinationPath "$Env:MHBoxDir\installArcAgent.ps1" -CreateFullPath -FileSource Host -Force
    ...
    #Invoke-Command -VMName $SQLvmName -ScriptBlock { powershell -File $Using:nestedVMMHBoxDir\installArcAgent.ps1 -accessToken $using:accessToken, -tenantId $Using:tenantId, -subscriptionId $Using:subscriptionId, -resourceGroup $Using:resourceGroup, -azureLocation $Using:azureLocation } -Credential $winCreds
    ...
    if ($amaExtension.StatusCode -eq 0) {
        ...
        # Verify if MHBox SQL resource is created
...
"@
    ...
    $sqlDefenderDcrFile = "$Env:MHBoxDir\defendersqldcrtemplate.json"
    ...
    $remoteScriptFileFile = "$Env:MHBoxDir\testDefenderForSQL.ps1"
    Copy-VMFile $SQLvmName -SourcePath "$Env:MHBoxDir\SqlAdvancedThreatProtectionShell.psm1" -DestinationPath "$Env:MHBoxDir\SqlAdvancedThreatProtectionShell.psm1" -CreateFullPath -FileSource Host -Force
    Copy-VMFile $SQLvmName -SourcePath "$Env:MHBoxDir\testDefenderForSQL.ps1" -DestinationPath $remoteScriptFileFile -CreateFullPath -FileSource Host -Force
...
#Set-JSDesktopBackground -ImagePath "$Env:MHBoxDir\wallpaper.bmp"
<#
...
if ($Env:flavor -eq 'ITPro') {
    ...
    & "$Env:MHBoxTestsDir\Invoke-Test.ps1"
