...
1. ODAA Subscription, which is a container which contains the different Oracle Database Services and defines the commercial aspects of the service
2. ODAA Database service. This are the Database services which are used to run Oracle Database on Azure
...
In additon the Subscriptions must have the following Microsoft Providers registered:
...
> NOTE: In case of the Microhack, the ODAA Subscription is already created and therefore your user does not needed the permissions.
...
The Marketplace does offer two purchasing options:
...
When you purchase ODAA Subscription through the public offer, you can choose between two licensing options:

1. bring your own license (BYOL) or
...
> **NOTE**: We need to select an Subscriptions on the screenshot, as shown. The ODAA Subscription himnself is not aligned to any Azure Region, but the Database Services created under the ODAA Subscription will be aligned to the Azure Region selected during onboarding.
...
To finish the ODAA Subscription onbording you will need an OCI account. This is needed because some components of the ODAA Databases needs to be configured via the Oracle Cloud Infrastructure (OCI) even if the physical Hardware runs inside Azure Data Centers.

You perform most onboarding tasks only one time, during your ODAA Subscription deployment. After you complete the onboarding tasks, you can begin provisioning and using ODAA Databanks resources in your Azure environment.
...
Find next how we linked our new created public offer ODAA Subscription with a new OCI account as an example.
...
2. Select the new created ODAA Subscription called "default".
![An image of the Azure portal showing the Oracle purchase details page.](media/odaa-subscription-select-default.jpeg)

3. On the Oracle Subscription detail page select the "Oracle Cloud Account" from the left-hand menu.
