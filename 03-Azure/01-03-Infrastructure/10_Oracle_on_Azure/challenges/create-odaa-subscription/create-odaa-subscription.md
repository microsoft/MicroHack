...
2. ODAA Database service. These are the Database services which are used to run Oracle Database on Azure
...
> **NOTE**: We need to select a Subscriptions on the screenshot, as shown. The ODAA Subscription itself is not aligned to any Azure Region, but the Database Services created under the ODAA Subscription will be aligned to the Azure Region selected during onboarding.
...
To finish the ODAA Subscription onboarding you will need an OCI account. This is needed because some components of the ODAA Databases needs to be configured via the Oracle Cloud Infrastructure (OCI) even if the physical Hardware runs inside Azure Data Centers.
...
Find next how we linked our newly created public offer ODAA Subscription with a new OCI account as an example.
