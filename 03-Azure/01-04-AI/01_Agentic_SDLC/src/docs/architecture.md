...
This site is a demo application written in TypeScript. The entire app was created originally from an [ERD diagram](../api-ts/ERD.png) and natural language prompts using Copilot. The frontend was created in the same way, using some of the design ideas in [the design folder](./design/).
...
- Declarative migrations (src/database/migrations) and deterministic seed data
...
- Database: SQLite (file located at `api-ts/data/app.db` by default; configurable via `DB_FILE`)
- Access: thin repository layer that maps camelCase models to snake_case columns
- Migrations: SQL scripts in `src/database/migrations` executed in order and tracked in a `migrations` table
