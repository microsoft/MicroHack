...
1. **Read the Zava data model.**
   ...
   |-------|-----------------|------|
   | `Inventory` | Current stock (`onHand`), `reorderPoint`, and `safetyStock` per product per store/warehouse | Challenges 1, 2 |
   | `DemandHistory` | Weekly sales units per product per location | Challenge 1 |
   | `ExternalSignals` | Pre-loaded market signals: competitor pricing, search trends, weather, news (sourced from Web IQ) | Challenge 1 |
   | `Products` | Product catalogue: `name`, `category`, `unitCost`, `leadTimeDays`, `supplierId` | Challenges 2, 3 |
   | `Stores` | Store and warehouse locations with region and type | Challenges 1, 2 |
   | `ReplenishmentOrders` | Purchase orders and transfers — status, approver, items JSON | Challenge 3 |
   | `Suppliers` | Supplier details | Challenge 3 |
...
2. **Explore the Foundry portal.**
   ...
   - Open **Tracing** in the left navigation — this is where you will inspect agent decisions in Challenges 2 and 3.
...
3. **Provision your own Fabric workspace and Data Agent.** *(~15 min)*
   ...
   **f. Copy your two IDs.**
   When it finishes, the **last cell prints your Workspace ID and Agent ID**. Note both — you paste them into the Fabric Data Agent tool in Challenge 1.
   ...
   **g. Verify your agent answers.** *(1 min)*
   ...
   > [!NOTE]
   > You attach the **Fabric Data Agent** as a *tool* on each Foundry agent, and the connection is created the **first time** you add it (Challenge 1). After that you simply **select** it. It points your Foundry agents at the data agent on top of **your** inventory Lakehouse.
...
- [ ] You have created your own Fabric workspace on your F2 capacity, run the setup notebook, and published your `inventory-hack-agent` Data Agent — and noted its **Workspace ID** and **Agent ID** (used in Challenge 1).
...
- Predict which tables Challenge 2 and Challenge 3 will lean on before you get there.
