...
> [!NOTE]
> This is an **optional stretch challenge** for teams that finish early. It builds directly on the three agents you created in Challenges 1–3. Skip it without penalty — Challenges 0–3 are the core of the hack.
...
You'll chain the three specialist agents you built in Challenges 1–3. **You do not need a separate orchestrator agent — the workflow *is* the orchestrator.**
...
5. On the **`replenishment-action-agent`** node, enable **Allow multi-turn conversation** — this lets the workflow pause at the purchase-order proposal and wait for the human's `YES` / `MODIFY` reply (the human-in-the-loop gate). In the **Loop condition** box, paste this PowerFx so the node keeps re-prompting until the agent submits or cancels the order:
   ...
   The loop runs **while the condition is true**: `MODIFY` re-presents the proposal (neither phrase present → keep looping), while `YES` (`✅ … submitted to ERP.`) or `NO` (`❌ Order cancelled…`) makes it false and ends the node. `Find` is **case-sensitive**, so keep the Challenge 3 confirmation wording exactly.
6. Click **Save**.
...
| The replenishment node never stops looping | The **Loop condition** `Find` is **case-sensitive** — keep the Challenge 3 confirmation wording (`submitted to ERP`, `Order cancelled`) exactly. |
