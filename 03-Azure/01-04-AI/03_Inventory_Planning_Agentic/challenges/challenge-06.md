# Challenge 6 — The Loop Reacts (Event-Driven, Stretch)
...
3. **Step 4 shows exactly what you're approving** — the per-line purchase order and the
   ...
   real `PO-…` is written to Cosmos, exactly as in Challenge 4. The automation ran
...
| Steps error out mid-run | The auto-run uses the same agents as Challenges 2–4 — make sure they work manually first, and that `az login` is valid. |
