...
| `ImportError: Blocked import of regex from current working directory …` when running `evaluators.py` | This is **NLTK's import guard** (NLTK is pulled in by `azure-ai-evaluation`), *not* an eval error — it fires before any row is scored. `git pull` the latest `src/evaluators.py` (it pre-imports `regex` to sidestep the guard). If you can't pull, run with the working directory off the import path: `PYTHONSAFEPATH=1 python src/evaluators.py` (equivalently `python -P src/evaluators.py`). |
