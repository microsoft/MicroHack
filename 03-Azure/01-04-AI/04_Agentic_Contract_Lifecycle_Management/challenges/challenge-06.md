...
> 🍴 **This is the one task that needs a fork.** Challenges 1–5 run in a **Codespace off this repo (no fork)** — but a CI gate lives in **GitHub Actions**, which only runs in **a repo you own**. So for this bonus, **fork the repo first** (repo page → **Fork** → **Create fork**), then do the steps below **in your fork**. Your Challenge 1 Codespace and `.env` still work for the optional OIDC stretch.
