...
> 🍴 **This is the one task that needs a fork.** Challenges 1–5 run in a **Codespace off this repo (no fork)**; a CI gate lives in **GitHub Actions**, which only runs in **a repo you own**. So **fork the repo** (repo page → **Fork** → **Create fork**) and do the steps below **in your fork**. The **required** enable + trigger is **100% GitHub web-UI clicks on the fork — no Codespace or terminal needed**. Only the **optional OIDC stretch** needs a terminal, and there your `git remote` must point at the fork (open a Codespace **on the fork**, or re-point `origin`); your Challenge 1 `.env` values and `az login` still apply.
