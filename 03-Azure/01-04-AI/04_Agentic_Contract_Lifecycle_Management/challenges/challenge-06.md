...
> 🍴 **This is the one task that needs a fork.** Challenges 1–5 run in a **Codespace off this repo (no fork)** — but a CI gate lives in **GitHub Actions**, which only runs in **a repo you own**. So for this bonus, **fork the repo first** (repo page → **Fork** → **Create fork**), then do the steps below **in your fork**. Your Challenge 1 `.env` values and `az login` still apply — but the Actions setup (and the optional `azd pipeline config`) must target **your fork**: open a Codespace **on the fork**, or re-point `git remote origin` to it, so azd configures the repo you own rather than the upstream you can't.
