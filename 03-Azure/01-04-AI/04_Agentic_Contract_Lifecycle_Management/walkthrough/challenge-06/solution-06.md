...
Steps in the attendee's **fork**:
1. **Enable the workflow** — scheduled workflows are **disabled by default in forks**. In **Actions → ci-eval**, click **Enable workflow** (yellow banner); otherwise it never runs (this is the #1 gotcha).
2. **Add the four repo secrets** — **Settings → Secrets and variables → Actions → New repository secret**. Enter each **separately**: the variable **name** goes in the **Name** box, and **only the raw value** in the **Secret** box — **no `NAME=` prefix, no quotes**. Getting each value (Codespace terminal, already `az login`'d): `AZURE_SUBSCRIPTION_ID` = `az account show --query id -o tsv`; `AZURE_TENANT_ID` = `az account show --query tenantId -o tsv`; `AZURE_AI_PROJECT_ENDPOINT` = already in `.env` (`grep AZURE_AI_PROJECT_ENDPOINT .env`, or `azd env get-values`, or Foundry portal **Overview → Endpoint**). `AZURE_CLIENT_ID` is the **client (app) ID** of an **OIDC-enabled** identity — `azure/login@v2` uses **federated** login, so that app needs a **federated credential** trusting `repo:<owner>/<repo>:ref:refs/heads/main` **plus** a role on the Foundry project (**Azure AI Developer** / **Cognitive Services User**). The clean one-shot that creates the identity, adds the federated credential, assigns roles, and sets the first three secrets is **`azd pipeline config --provider github`**; then add `AZURE_AI_PROJECT_ENDPOINT` manually. Missing secrets → the job runs but skips the gates (green no-op), which still satisfies the criteria.
3. **Trigger** via **Run workflow** (`workflow_dispatch`) or the nightly schedule.

✅ Green check = gates passed; red X = a regression tripped a gate (the whole point).
...
| Azure login step fails: `AADSTS70025: The client '…' has no configured federated identity credentials` | `AZURE_CLIENT_ID` points at an app with no **federated credential** for the repo (often the Ch1 SharePoint app "CLM Microhack Corpus", which uses a secret, not OIDC). Add a federated credential — **Entra → App registrations → *app* → Federated credentials → Add → GitHub Actions**, Entity **Branch** = `main` (subject `repo:<owner>/<repo>:ref:refs/heads/main`, audience `api://AzureADTokenExchange`) — or `az ad app federated-credential create --id <AZURE_CLIENT_ID> --parameters '{"name":"gh-ci-eval-main","issuer":"https://token.actions.githubusercontent.com","subject":"repo:<owner>/<repo>:ref:refs/heads/main","audiences":["api://AzureADTokenExchange"]}'`. Then give the app the **Azure AI Developer** role on the project. `azd pipeline config --provider github` does the whole thing at once. |
