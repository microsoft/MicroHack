...
2. **Add repo secrets** (**Settings → Secrets and variables → Actions**): `AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`, `AZURE_AI_PROJECT_ENDPOINT`. Missing → the job runs but skips the gates (green no-op).
